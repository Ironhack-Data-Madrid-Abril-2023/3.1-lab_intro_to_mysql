...
CREATE TABLE IF NOT EXISTS `3.1-lab_intro_to_mysql`.`invoice` (
  ...
  PRIMARY KEY (`Invoice_ID`, `salesperson_idstaff`, `customer_customer_id`),
  INDEX `fk_invoice_salesperson_idx` (`salesperson_idstaff` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `3.1-lab_intro_to_mysql`.`cars` (
  ...
  `invoice_salesperson_idstaff` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`ID_cars`, `invoice_Invoice_ID`, `invoice_salesperson_idstaff`),
  INDEX `fk_cars_invoice1_idx` (`invoice_Invoice_ID` ASC, `invoice_salesperson_idstaff` ASC) VISIBLE,
  CONSTRAINT `fk_cars_invoice1`
    FOREIGN KEY (`invoice_Invoice_ID` , `invoice_salesperson_idstaff`)
    REFERENCES `3.1-lab_intro_to_mysql`.`invoice` (`Invoice_ID` , `salesperson_idstaff`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
